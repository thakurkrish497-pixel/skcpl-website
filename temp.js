...
// These are the specific spots on the website that the father is allowed to edit.
...
// Auto-listen to state changes (so if they log in/out in another tab, it updates)
if (supabase) {
...
async function attemptLogin() {
  ...
  try {
    ...
    if (error) {
      ...
      emailInput.value = '';
      passwordInput.value = '';
      // The onAuthStateChange listener will automatically call showDashboard
    }
  } catch (err) {
...
async function loadDashboardImages() {
  ...
  try {
    ...
    // Map them by key for easy lookup
    const recordMap = {};
    if (records) {
    ...
    // Render a card for each editable image
...
async function handleImageUpload(event, key) {
  ...
  try {
    ...
    // Reload dashboard to show the new image
    loadDashboardImages();
